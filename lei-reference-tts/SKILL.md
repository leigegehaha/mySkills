---
name: lei-reference-tts
description: Generate speech with Lei's cloned voice using the local IndexTTS2 repository. Use when the user asks to synthesize audio with lei's voice, Lei voice, 用 lei 的声音生成语音, 用 lei 配音, or when the task is to turn a long draft into a polished oral script with pauses, fillers, emotion-based segmentation, per-segment emotion parameters, and one merged long-form wav. Outputs should be saved under the repository path `outputs/lei/`.
---
...
```bash
python3 /Users/zhangleiandhim/.codex/skills/lei-reference-tts/scripts/synthesize.py --text "你好，这是一段 lei 音色测试。"
```
...
```bash
python3 /Users/zhangleiandhim/.codex/skills/lei-reference-tts/scripts/scaffold_long_form_project.py \
...
```bash
python3 /Users/zhangleiandhim/.codex/skills/lei-reference-tts/scripts/synthesize_project.py \
...
```bash
python3 /Users/zhangleiandhim/.codex/skills/lei-reference-tts/scripts/synthesize.py \
  --text "这里放一大段长文稿……" \
...
- 可直接编辑配置文件：`/Users/zhangleiandhim/.codex/skills/lei-reference-tts/emotion_config.json`
- 支持三种模式：
  - `reference`：情绪参考音频
