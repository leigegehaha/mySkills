...
function getElapsedSeconds(item) {
  if (Number.isFinite(Number(item.totalElapsedSeconds))) {
    return Number(item.totalElapsedSeconds);
  }
...
function render() {
  ...
  if (!state.items.length) {
    historyGrid.innerHTML = '<div class="placeholder">还没有历史记录。提交一次任务，流式卡片会出现在这里。</div>';
    return;
  }
      ...
      <article class="card status-${escapeHtml(item.status)}">
        ...
        <div class="card-body">
          ...
          <div class="meta">
            ...
            <span>时长 · ${escapeHtml(item.duration || 6)} 秒</span>
            ${elapsedSeconds != null ? `<span>生成耗时 · ${escapeHtml(elapsedSeconds)} 秒</span>` : ''}
            ${item.model ? `<span>模型 · ${escapeHtml(item.model)}</span>` : ''}
...
function reuseItem(jobId) {
  ...
  aspectRatioInput.value = item.aspectRatio || '3:2';
