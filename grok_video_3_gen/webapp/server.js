...
const {
  ...
  normalizeDuration,
  getModelByDuration,
} = require('../scripts/video-generator');
...
function deleteHistoryItem(jobId) {
  ...
  if (item.imagePath && String(item.imagePath).startsWith(UPLOAD_ROOT)) {
  ...
  while (activeJobs.has(job.jobId)) {
    try {
      ...
      if (['completed', 'success', 'succeeded', 'finished'].includes(status) || videoUrl) {
        ...
        let localPath = null;
        if (videoUrl) {
          localPath = createOutputPath(job.taskId);
        ...
        updateHistoryItem(job.jobId, {
          ...
          remoteUrl: videoUrl || null,
          localPath,
          videoPath: localPath ? `/output/${path.basename(localPath)}` : null,
          completedAt,
...
async function handleGenerate(req, res) {
  try {
    ...
    const aspectRatio = String(body.aspectRatio || DEFAULT_RATIO);
    const size = String(body.size || DEFAULT_SIZE).toUpperCase();
    const duration = normalizeDuration(body.duration || DEFAULT_DURATION);
    ...
    if (mode === 'image' && !imageSource) {
      sendJson(res, 400, { error: '图生视频需要本地图片或远程 URL' });
      return;
    }
    ...
    const { taskId } = await createVideo(prompt, mode === 'image' ? imageSource : null, aspectRatio, size, duration);
    ...
    watchJob({ jobId, taskId, createdAt }).catch((error) => {
      updateHistoryItem(jobId, {
        status: 'failed',
        error: error.message,
      });
...
async function handleDeleteHistory(req, res) {
  try {
    ...
    if (!jobId) {
      sendJson(res, 400, { error: '缺少 jobId' });
      return;
    }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/api/preview') {
    ...
    serveFile(res, path.resolve(requested));
    return;
  }

  if (req.method === 'GET' && url.pathname.startsWith('/output/')) {
    const filePath = path.join(OUTPUT_ROOT, path.basename(url.pathname));
...
server.listen(PORT, HOST, () => {
  console.log(`Grok Video Tester running at http://${HOST}:${PORT}`);
});
